...
Impresor::Impresor(QWidget *parent) : QWidget(parent), ui(new Ui::Impresor), selectedIndex(0)  {
    ...
    timerDir->setInterval( Comun::getInstance()->getTimerDir() );

    timerImpresion = new QTimer(this);
    connect( timerImpresion, SIGNAL( timeout() ), this, SLOT( slot_timerImpresion() ) );

    // Cada este tiempo tira una impresion
    timerImpresion->setInterval( Comun::getInstance()->getTimerimpresiones() );
...
void Impresor::iniciar()  {

    // El timerDir tambien lo invoca. Pero lo actualizamos tambien antes que timerDir llegue al timeout
...
void Impresor::setTimerImpresiones( int milisegundos )
{
    timerImpresion->setInterval( milisegundos );
}

void Impresor::setTimerDir(int milisegundos)
{
    timerDir->setInterval( milisegundos );
}

void Impresor::cancelar()
{
    timerDir->stop();
    timerImpresion->stop();
}
...
void Impresor::slot_itemChanged(const QModelIndex &current, const QModelIndex &)  {
    ...
    if ( is_impreso == "1" )  {
        archivoImagen = "../fotos/" + Comun::getInstance()->getHashTag() + conPlantilla + nombreFoto;
    }
    else  {
        archivoImagen = "../fotos/" + Comun::getInstance()->getHashTag() + sinPantilla + nombreFoto;
    }

    emit signal_fotoEnGaleria(archivoImagen);
    emit signal_fotoOriginalEnGaleria("../fotos/" + Comun::getInstance()->getHashTag() + "/" + nombreFoto);
...
void Impresor::slot_timerImpresion()  {
    ...
    qDebug() << "slot_timerImpresion" << Comun::getInstance()->getImpresion();

    this->slot_updateTabla();

    // Para que no imrpima nada, ni en PDF ni en la impresora
    if ( Comun::getInstance()->getImpresion() == Comun::NOPDF_NOPRINTER )  {
        return;
    }

    // Obtiene la nueva foto para imprimir y lo setea como impreso en la tabla
    QString fotoParaImprimir = AdminDB::getInstance()->getFotoParaImprimir( Comun::getInstance()->getHashTag() );

    // Si esta vacio es porque no hay foto para imprimir, o la carpeta esta vacia o se imprimieron
    // todas, o inconsistencia
    if ( fotoParaImprimir.isEmpty() )
        return;

    QString file = "../fotos/" + Comun::getInstance()->getHashTag() + "/" + fotoParaImprimir;
    QFileInfo fileInfo(file);
    qDebug() << "se imprime" << fileInfo.baseName();

    QPrinter printer(QPrinter::PrinterResolution);

    if ( Comun::getInstance()->getSizeHoja() == Comun::SIZE15x10 )
...
//    printer.setFullPage(true);  // Para que imprimir en la hoja completa
    ...
    if ( Comun::getInstance()->getImpresion() == Comun::IMPRIMIR )  {  // Para que imprima
        printer.setOutputFormat(QPrinter::NativeFormat);
    }
    else if ( Comun::getInstance()->getImpresion() == Comun::PDF )  {  // Para que solo guarde en PDF
        printer.setOutputFileName("../fotos/" + Comun::getInstance()->getHashTag() + "_impresiones/" + fileInfo.baseName() + ".pdf");
    ...
    QImage imPlantilla;
    if ( ! Comun::getInstance()->getArchivoPlantilla().isEmpty() )  {
        bool isCargado = imPlantilla.load("../plantillas/" + Comun::getInstance()->getArchivoPlantilla(), "JPG");
        qDebug() << "Plantilla " << "../plantillas/" + Comun::getInstance()->getArchivoPlantilla() << "  // Se cargo JPG ???" << isCargado;

        if ( isCargado == false )  {
            isCargado = imPlantilla.load("../plantillas/" + Comun::getInstance()->getArchivoPlantilla(), "PNG");
            qDebug() << "Plantilla " << "../plantillas/" + Comun::getInstance()->getArchivoPlantilla() << "  // Se cargo PNG ???" << isCargado;
        }
    }
...
//    int bottomFoto = printer.height() / 8;
    ...
    papel.save("../fotos/" + Comun::getInstance()->getHashTag() + "_imagenes/" + fileInfo.baseName() + ".jpg");
}
...
//    delete garbageCollectorTableModelProductos;
    ...
    qDebug() << "inicio de slot_updateTabla" << QTime::currentTime().toString("hh:mm:ss");
...
//    garbageCollectorTableModelProductos = tableModel;
    ...
    QString filtro;
    filtro = "id_fiesta like '" + AdminDB::getInstance()->getIdFiesta( Comun::getInstance()->getHashTag() ) + "'";
    ...
    tableModel->setHeaderData(5, Qt::Horizontal, "Impreso");
...
//    ui->tabla->setColumnHidden(1, true);
    ...
    qDebug() << "FIN de slot_updateTabla" << QTime::currentTime().toString("hh:mm:ss");
}
