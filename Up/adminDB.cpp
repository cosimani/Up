...
// Cada vez que se busca un hashtag, sea nuevo o ya existente. Se invoca a este metodo y este se encarga
// de ver si en la base ya existe este hashtag y se encarga de acomodar todo.
...
//    int index_fecha = querySelect.record().indexOf("fecha");
    ...
    if ( contadorRows == 0 )  {  // No existe, entonces agregamos registro
        ...
        return queryInsert.exec("INSERT INTO fiestas (nombre, fecha, hashtag) VALUES ('"
...
bool AdminDB::addIdAlbum(QString hashtag, QString id_album)  {
    ...
    // Si se cumple es porque hay inconsistencia, hay que agregar primero el registro a fiestas o
    // hay mas de un registro con el mismo hashtag
    if ( contadorRows != 1)  {
        qDebug() << "Hay inconsistencia, hay que agregar primero el registro a fiestas o hay "
                    "mas de un registro igual";
        return false;
    }
    else  {
        ...
        if ( contadorRows == 0 )  {
            QSqlQuery queryUpdate(db);
            return queryUpdate.exec("UPDATE fiestas SET id_album='" + id_album
...
QString AdminDB::getFotoParaImprimir(QString hashtag)  {
    ...
    if ( contadorRows != 1)  {
        ...
        // Buscamos la foto mas antigua sin imprimir.
        QSqlQuery querySelectFoto(db);
